...
export function BoldTemplate({ data, color, font }: Props) {
  ...
  const renderSection = (sectionKey: string) => {
        ...
        <div key={sectionKey} className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {data.skills.map((skill) => (
              <span key={skill.id} className="px-3 py-1 font-bold text-xs text-white" style={{ backgroundColor: color, wordSpacing: "0.05em" }}>
                {skill.name}
              </span>
            ))}
          </div>
    ...
    return (
      <div key={sectionKey} className="mb-6">
        <h2 className="text-lg font-black uppercase mb-3" style={{ color }}>{customSection.title}</h2>
    ...
    <div className={`w-full h-full bg-white ${fontClass}`} style={{ fontSize: "11px" }}>
      ...
      <div className="p-8 pt-6">
        <div className="flex flex-wrap gap-4 text-xs mb-6 pb-4 border-b-2" style={{ borderColor: color }}>
          ...
          {data.personal.location && <span className="font-medium" style={{ wordSpacing: "0.05em" }}>{data.personal.location}</span>}
        </div>
